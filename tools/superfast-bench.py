#!/usr/bin/env python3
"""superfast-bench — llama-bench-shaped sweep over superfast's HTTP endpoint.

    superfast-bench -p 512,2048,8192 -n 128 -d dflash2,mtp -r 3
...
questions and BOTH are needed:
    ...
    superfast-bench.py a SIZE SWEEP at fixed synthetic lengths, so a number
...
def main():
    ap = argparse.ArgumentParser(
        description="llama-bench-shaped pp/tg sweep over superfast's endpoint")
    ...
    print("superfast-bench: %s, ctx %s, reps %d, effort %s"
    ...
    for drafter in drafters:
        ...
        for target in tgs:
            ...
            #
            # superfast's decode rate depends on draft ACCEPTANCE, which
            # depends on how predictable the text is. Measured on one build,
            # holding everything else fixed and varying only the prompt: prose
            # 20.7 / 22.6 t/s, code 40.4, proof 41.6. A 2x spread. The first
            # cut of this file used a single "write an essay" prompt — prose,
            # the WORST case — and so understated dflash2 by ~30% against its
            # own ten-shape mean. That is the same mistake this project has

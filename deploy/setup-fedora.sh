...
#
# STATUS (2026-09-09):
#   Phases 1-8  VALIDATED on the reference host — every command below was run
#               and verified there; see docs/fedora-44-setup.md for the log.
#               Phase 8 installs superfast.service (systemd user unit) and
#               waits for /health.
#
...
phase_weights() {
    log "== phase 6/8: checkpoint download (curl -C -, exact-offset resume) =="
    mkdir -p "$MODELS_DIR"
...
phase_smoke() {
    log "== phase 7b/8: container device test (SMOKE=1) =="
    podman run --rm --device /dev/kfd --device /dev/dri docker.io/library/fedora:44 \
        ls -l /dev/kfd /dev/dri
}

phase_engine() {
    log "== phase 8/8: engine service (systemd user unit) =="
...
main() {
    ...
    phase_engine
    log "setup complete — SUPERFAST serving on http://<host>:8731"
}
